...
const Layout = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    innerWidth = 901;
    menu = false;
    $$rendered = ` ${$$result.head += `<!-- HEAD_svelte-1w6uokn_START -->${$$result.title = `<title>To Bitoni | ${escape(pages[$store.index])}</title>`, ""}<meta property="og:description" content="A magazine"><link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Inter"><link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Courier Prime"><link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Abhaya Libre"><!-- HEAD_svelte-1w6uokn_END -->`, ""} <main class="h-full">${menu ? `${validate_component(Menu, "Menu").$$render($$result, { toggleMenu, innerWidth }, {}, {})}` : ``} ${innerWidth > 900 ? `${validate_component(Nav, "Top").$$render(
      $$result,
      { index: $store.index },
      {
        index: ($$value) => {
